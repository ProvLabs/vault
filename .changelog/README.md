...
```md
* Fix the thing that was broken [PR 123](https://github.com/provlabs/vault/pull/123).
```
...
Here too, if there is an issue number for the change, you should use the `--issue-no` (or `-n`) flag instead of `--pull-request` (or `-p`) to have the correct links.
...
If the entry for that change is updated by a later PR, there's a very good chance the release branch will not get the follow-up.
