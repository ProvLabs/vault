...
> creation seed, and the `uylds.fcc` NAV-check exemption no longer have any effect. The wire fields
> remain for client compatibility (`payment_denom` always equals `underlying_asset`; `redeem_denom`
> must be empty or the underlying asset); field deletion is deferred to a future major release.
