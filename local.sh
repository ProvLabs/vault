...
if ! [ -f .vaulty/data/priv_validator_state.json ]; then
  "$SIMD_BIN" init validator --chain-id "vaulty-1" --home .vaulty &> /dev/null
  ...
  "$SIMD_BIN" genesis gentx validator 1000000ustake --chain-id "vaulty-1" --home .vaulty --keyring-backend test &> /dev/null
