...
if [[ -n "$v_rc" && -z "$have_summary" ]]; then
    printf 'This is Provlab vault module release candidate `%s`.\n' "$version" > "$unreleased_sum_file"
fi
...
while IFS="" read -r line || [[ -n "$line" ]]; do
    if [[ "$line" =~ ^##[[:space:]]+Unreleased[[:space:]]*$ ]]; then
        printf '## [%s](https://github.com/provlabs/vault/releases/tag/%s) %s\n' "$version" "$version" "$date" >> "$cur_file"
...
[[ -n "$prev_ver_rc" ]] && printf '* https://github.com/provlabs/vault/compare/%s...%s\n' "$prev_ver_rc" "$version" >> "$new_cl_entry_file"
printf '* https://github.com/provlabs/vault/compare/%s...%s\n\n' "$prev_ver" "$version" >> "$new_cl_entry_file"
