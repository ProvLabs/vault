...
func (m MsgCreateVaultRequest) ValidateBasic() error {
	...
	if err := ValidateInitialPaymentNAV(m.InitialPaymentNav); err != nil {
...
// ValidateInitialPaymentNAV rejects any non-nil initial payment NAV. Vaults are
// created single-denom, so a bootstrap NAV entry is never needed.
func ValidateInitialPaymentNAV(nav *InitialVaultNAV) error {
	if nav != nil {
		return fmt.Errorf("initial_payment_nav must be omitted: payment_denom must be empty or equal underlying_asset")
	}
	return nil
}
