...
func (k msgServer) VaultSettleAssetPayment(goCtx context.Context, msg *types.MsgVaultSettleAssetPaymentRequest) (*types.MsgVaultSettleAssetPaymentResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to fetch exchange payment details: %w", err)
	}
	...
	if _, err := k.ExchangeKeeper.AcceptPayment(markertypes.WithBypass(ctx), exchangeMsg); err != nil {
