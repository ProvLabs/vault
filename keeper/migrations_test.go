...
func (s *TestSuite) TestKeeper_MigrateAUMFeeParams() {
	createVault := func(shareDenom string, aumFeeBips uint32) (sdk.AccAddress, *types.VaultAccount) {
		addr := types.GetVaultAddress(shareDenom)
		vault := &types.VaultAccount{
			BaseAccount:            authtypes.NewBaseAccountWithAddress(addr),
			Admin:                  s.adminAddr.String(),
			TotalShares:            sdk.NewCoin(shareDenom, sdkmath.ZeroInt()),
			UnderlyingAsset:        "uusdc",
			PaymentDenom:           "uusdc",
			CurrentInterestRate:    types.ZeroInterestRate,
			DesiredInterestRate:    types.ZeroInterestRate,
			SwapInEnabled:          true,
		...
		acct := s.simApp.AccountKeeper.NewAccount(s.ctx, vault)
		vaultAcct, ok := acct.(*types.VaultAccount)
		s.Require().True(ok, "new account should return a *types.VaultAccount")
		s.simApp.AccountKeeper.SetAccount(s.ctx, vaultAcct)
		return addr, vaultAcct
	}

	vaultAddrA, _ := createVault("vaultA", 0)
	vaultAddrB, _ := createVault("vaultB", 100)

	// Set legacy AUM fee recipient address in the KV store.
	legacyAddr := sdk.AccAddress([]byte("legacy-tech-fee-addr________"))
	kvStore := s.simApp.VaultKeeper.OpenKVStore(s.ctx)
	kvStore.Set(types.AUMFeeAddressKeyPrefix, legacyAddr)

	// Remove params to trigger migration logic that initializes them from legacy store.
	err := s.simApp.VaultKeeper.Params.Remove(s.ctx)
	s.Require().NoError(err, "failed to remove params")

	err = s.simApp.VaultKeeper.MigrateAUMFeeParams(s.ctx)
	s.Require().NoError(err, "MigrateAUMFeeParams should not return an error")

	// Check params
	params, err := s.simApp.VaultKeeper.Params.Get(s.ctx)
	s.Require().NoError(err, "params should be initialized after migration")
	s.Require().Equal(legacyAddr.String(), params.TechFeeAddress, "TechFeeAddress should be migrated from legacy prefix")
	s.Require().Equal(uint32(types.DefaultAumFeeBips), params.DefaultAumFeeBips, "DefaultAumFeeBips should be set to default")

	// Check vaults
	gotA := s.simApp.AccountKeeper.GetAccount(s.ctx, vaultAddrA)
	gotVaultA := gotA.(*types.VaultAccount)
	s.Require().Equal(uint32(types.DefaultAumFeeBips), gotVaultA.AumFeeBips, "vault A AumFeeBips should be updated to default when 0")

	gotB := s.simApp.AccountKeeper.GetAccount(s.ctx, vaultAddrB)
	gotVaultB := gotB.(*types.VaultAccount)
	s.Require().Equal(uint32(100), gotVaultB.AumFeeBips, "vault B AumFeeBips should remain unchanged when non-zero")
}
