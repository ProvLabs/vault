...
| `UpdateVaultNAV`         | NAV authority only                |                   ✅ |                 ✅ | Upserts the internal NAV entry, which stays internal to the vault and is never published to the marker module. Reconciles first when unpaused; leaves `PausedBalance` frozen when paused, so a pause-reprice-unpause sequence cannot be front-run and the new price takes effect at unpause. |
...
The entry is written to the vault's own table only. It is **not** published to the **marker module**: a vault holds no permission on the markers of the assets it prices, and vault creation is permissionless, so mirroring these prices would let any vault overwrite the chain-wide NAV of an arbitrary registered marker outside the access controls `x/marker` enforces on its own NAV messages. The vault's share marker is the one marker whose NAV the module publishes (see `01_concepts.md`).
