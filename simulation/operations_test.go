...
func (s *VaultSimTestSuite) SetupTest() {
	...
	err := simulation.CreateGlobalMarker(s.ctx, s.app.AccountKeeper, s.app.BankKeeper, s.app.MarkerKeeper, sdk.NewInt64Coin("underlyingvaulty", 100_000_000), s.accs, true)
	s.Require().NoError(err, "CreateGlobalMarker underlying")
	err = simulation.CreateGlobalMarker(s.ctx, s.app.AccountKeeper, s.app.BankKeeper, s.app.MarkerKeeper, sdk.NewInt64Coin("paymentvaulty", 100_000_000), s.accs, true)
...
func (s *VaultSimTestSuite) TestSimulateMsgSwapIn() {
	...
	err := simulation.CreateVault(s.ctx, s.app.VaultKeeper, s.app.AccountKeeper, s.app.BankKeeper, s.app.MarkerKeeper, "underlyingvaulty", "", "underlyingshare", selected, s.accs)
...
func (s *VaultSimTestSuite) TestSimulateMsgSwapOut() {
	...
	err = simulation.SwapIn(s.ctx, s.app.VaultKeeper, selected, "underlyingshare", sdk.NewInt64Coin("underlyingvaulty", 100))
...
func (s *VaultSimTestSuite) TestSimulateMsgExpeditePendingSwapOut() {
	...
	err := simulation.CreateVault(s.ctx, s.app.VaultKeeper, s.app.AccountKeeper, s.app.BankKeeper, s.app.MarkerKeeper, "underlyingvaulty", "", "underlyingshare", admin, s.accs)
	s.Require().NoError(err, "CreateVault")
	err = simulation.AddAttribute(s.ctx, user.Address, simulation.RequiredMarkerAttribute, s.app.NameKeeper, s.app.AttributeKeeper)
	s.Require().NoError(err, "AddAttribute")
	err = simulation.SwapIn(s.ctx, s.app.VaultKeeper, user, "underlyingshare", sdk.NewInt64Coin("underlyingvaulty", 100))
	s.Require().NoError(err, "SwapIn")
	shares := s.app.BankKeeper.GetBalance(s.ctx, user.Address, "underlyingshare")
	err = simulation.SwapOut(s.ctx, s.app.VaultKeeper, user, shares, "underlyingvaulty")
...
func (s *VaultSimTestSuite) TestSimulateMsgWithdrawInterestFunds() {
	...
	err := simulation.CreateVault(s.ctx, s.app.VaultKeeper, s.app.AccountKeeper, s.app.BankKeeper, s.app.MarkerKeeper, "underlyingvaulty", "", "underlyingshare", admin, s.accs)
	s.Require().NoError(err, "CreateVault")
	_, err = simulation.DepositInterestFunds(s.ctx, s.app.VaultKeeper, "underlyingshare", sdk.NewInt64Coin("underlyingvaulty", 100))
...
func (s *VaultSimTestSuite) TestSimulateMsgWithdrawPrincipalFunds() {
	...
	err = simulation.PauseVault(s.ctx, s.app.VaultKeeper, "underlyingshare")
	s.Require().NoError(err, "PauseVault")
	_, err = simulation.DepositPrincipalFunds(s.ctx, s.app.VaultKeeper, "underlyingshare", sdk.NewInt64Coin("underlyingvaulty", 100))
