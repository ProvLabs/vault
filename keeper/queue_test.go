...
func TestEnqueueDequeue_Start(t *testing.T) {
	...
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, addr), "enqueue payout verification for %s should succeed", addr.String())

	it, err := k.PayoutVerificationQueue.Iterate(stdCtx, nil)
	...
	require.NoError(t, k.DequeuePayoutVerification(stdCtx, addr), "dequeue payout verification for %s should succeed", addr.String())

	it2, err := k.PayoutVerificationQueue.Iterate(stdCtx, nil)
...
func TestEnqueueDequeue_Timeout(t *testing.T) {
	...
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, ts, addr), "enqueue payout timeout (%d) for %s should succeed", ts, addr.String())
	...
	require.NoError(t, k.DequeuePayoutTimeout(stdCtx, ts, addr), "dequeue payout timeout (%d) for %s should succeed", ts, addr.String())

	it2, err := k.PayoutTimeoutQueue.Iterate(stdCtx, nil)
...
func TestWalkDueStarts(t *testing.T) {
	...
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, a1), "enqueue payout verification for a1 should succeed")
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, a2), "enqueue payout verification for a2 should succeed")
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, a1), "enqueue payout verification duplicate for a1 should succeed (set semantics)")

	var seen []string
	require.NoError(t, k.WalkPayoutVerifications(stdCtx, func(addr sdk.AccAddress) (bool, error) {
...
func TestWalkDueStarts_StopEarly(t *testing.T) {
	...
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, a), "enqueue payout verification for a should succeed")
	require.NoError(t, k.EnqueuePayoutVerification(stdCtx, b), "enqueue payout verification for b should succeed")

	calls := 0
	require.NoError(t, k.WalkPayoutVerifications(stdCtx, func(_ sdk.AccAddress) (bool, error) {
		calls++
		return true, nil
...
func TestWalkDueStarts_ErrorPropagates(t *testing.T) {
	...
	err := k.WalkPayoutVerifications(stdCtx, func(_ sdk.AccAddress) (bool, error) {
...
func TestWalkDueTimeouts(t *testing.T) {
	...
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, 50, a1), "enqueue payout timeout (50) for a1 should succeed")
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, 75, a2), "enqueue payout timeout (75) for a2 should succeed")
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, 500, a1), "enqueue payout timeout (500) for a1 should succeed")

	var seen []uint64
	require.NoError(t, k.WalkDuePayoutTimeouts(stdCtx, 100, func(ts uint64, _ sdk.AccAddress) (bool, error) {
		seen = append(seen, ts)
		return false, nil
...
func TestWalkDueTimeouts_StopEarly(t *testing.T) {
	...
	a := sdk.MustAccAddressFromBech32(utils.TestAddress().Bech32)
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, 10, a), "enqueue payout timeout (10) should succeed")
	require.NoError(t, k.EnqueuePayoutTimeout(stdCtx, 20, a), "enqueue payout timeout (20) should succeed")

	calls := 0
	require.NoError(t, k.WalkDuePayoutTimeouts(stdCtx, 25, func(_ uint64, _ sdk.AccAddress) (bool, error) {
		calls++
		return true, nil
...
func TestWalkDueTimeouts_ErrorPropagates(t *testing.T) {
	...
	errBoom := errors.New("boom")
	err := k.WalkDuePayoutTimeouts(stdCtx, 25, func(_ uint64, _ sdk.AccAddress) (bool, error) {
		return false, errBoom
	})
	require.ErrorIs(t, err, errBoom, "walk should propagate callback error")
}
...
func TestRemoveAllStartsForVault(t *testing.T) {
	...
	require.NoError(t, k.DequeuePayoutVerification(stdCtx, a1), "dequeue payout verification for a1 should succeed")
